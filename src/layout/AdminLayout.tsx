...
// --- Styles Helper: 大字体 + 舒适间距 ---
const linkStyle = ({ isActive }: { isActive: boolean }) => ({
  display: "flex",
  alignItems: "center",
  gap: 14,                // 图标和文字距离加大
  padding: "12px 16px",   // 点击区域变大，更易点
  borderRadius: 12,       // 更大的圆角，更有现代感
  textDecoration: "none",
  color: isActive ? "#2563eb" : "#475569", 
  background: isActive ? "#eff6ff" : "transparent",
  fontSize: 15,           // ✨ 字体加大到 15px
  fontWeight: isActive ? 600 : 500,
  transition: "all 0.2s ease-in-out", // 添加平滑过渡
  marginBottom: 4         // 菜单项之间增加一点空隙
});
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100%" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 28, paddingLeft: 8 }}>
        <div style={{
          width: 42, height: 42, borderRadius: 12, // Logo 变大
          background: "linear-gradient(135deg, #3b82f6, #1d4ed8)", // 更鲜艳的渐变
          color: "white", display: "grid", placeItems: "center",
          fontWeight: 800, fontSize: 20, 
          boxShadow: "0 4px 12px rgba(37, 99, 235, 0.25)" // 增加发光阴影
        }}>
          E
        </div>
        <div>
      ...
      <div style={{ 
        ...
        <div style={{
          width: 52, height: 52, borderRadius: "50%", // 头像变大
        ...
        <div style={{ overflow: "hidden" }}>
          ...
          <div style={{ fontSize: 13, color: "#64748b" }}>{t("admin.role")}</div>
        </div>
      </div>

      {/* --- Navigation: 图标和字体同步放大 --- */}
      <nav style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        <NavLink to="/admin/dashboard" style={linkStyle} onClick={handleNavClick}>
          <LayoutDashboard size={20} /> {t("common.dashboard")}
        </NavLink>
        <NavLink to="/admin/students" style={linkStyle} onClick={handleNavClick}>
          <Users size={20} /> {t("common.students")}
        </NavLink>
        <NavLink to="/admin/attendance" style={linkStyle} onClick={handleNavClick}>
          <ClipboardCheck size={20} /> {t("common.attendance")}
        </NavLink>
        <NavLink to="/admin/materials" style={linkStyle} onClick={handleNavClick}>
          <BookOpen size={20} /> {t("common.materials")}
        </NavLink>
        <NavLink to="/admin/feedback" style={linkStyle} onClick={handleNavClick}>
          <MessageSquare size={20} /> {t("common.feedback")}
        </NavLink>
        <NavLink to="/admin/progress" style={linkStyle} onClick={handleNavClick}>
          <TrendingUp size={20} /> {t("common.progress")}
        </NavLink>
      </nav>

      {/* --- Footer Actions: 卡片式按钮 --- */}
        ...
        <header style={{ 
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ textAlign: "right", lineHeight: 1.3 }}>
              ...
              <div style={{ fontSize: 12, color: "#64748b" }}>{t("admin.role")}</div>
            </div>
          </div>
        </header>
